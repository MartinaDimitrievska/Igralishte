                ...
                <form method="POST" action="{{ route('products.update', $product->id) }}" enctype="multipart/form-data">
                        ...
                        <div class="mb-2">
                            <select name="status" id="status" class="rounded-md border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                ...
                                @foreach ($statuses as $status)
                                    <option value="{{ $status->id }}" {{ $product->status->id == $status->id ? 'selected' : '' }}>{{ $status->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="mb-2">
                            ...
                            <x-input type="text" name="name" id="name" class="mt-1 p-2 border rounded-md w-full" value="{{ $product->name }}" autofocus />
                        </div>
                        <div class="mb-2">
                            <x-label for="description" class="block text-sm font-medium text-gray-700" :value="__('Опис')" />
                            <textarea name="description" id="description" rows="3" class="mt-1 p-2 border rounded-md w-full rounded-lg border-gray-300 shadow-sm">{{ $product->description }}</textarea>
                        </div>
                        <div class="mb-2">
                            <x-label for="price" class="block text-sm font-medium text-gray-700" :value="__('Цена')" />
                            <x-input type="text" name="price" id="price" class="mt-1 p-2 border rounded-md w-full" value="{{ $product->price }}" />
                        </div>
                        <div class="mb-2">
                            ...
                            <div class="flex items-center">
                                ...
                                <input type="number" id="quantity" name="quantity" class="w-11 no-scrollbar pl-4 pr-1 border-none" value="{{ $product->quantity }}" min="1">
                        ...
                        <div>
                                ...
                                <div class="flex">
                                    @foreach ($sizes as $size)
                                        <label for="size_{{ $size->id }}" class="size-label">
                                            <input type="checkbox" id="size_{{ $size->id }}" name="sizes[]" value="{{ $size->id }}" class="hidden" {{ in_array($size->id, $product->sizes->pluck('id')->toArray()) ? 'checked' : '' }}>
                                            <div class="size-square {{ in_array($size->id, $product->sizes->pluck('id')->toArray()) ? 'selected' : '' }}" style="background-color: #FFDBDB">{{ $size->name }}</div>
                                        </label>
                                    @endforeach
                                </div>
                            </div>
                            <div class="mt-2">
                                ...
                                <textarea id="size_advice" name="size_advice" rows="3" class="mt-1 rounded-md border-gray-300 w-full focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">{{ $product->size_advice }}</textarea>
                            </div>
                        </div>
                        <div>
                            ...
                            <div class="flex flex-wrap mb-4">
                                @foreach ($colors as $color)
                                    ...
                                    @endphp
                                    <label for="color_{{ $color->id }}" class="colors inline-block w-6 h-6 mr-2 mt-1 rounded-lg cursor-pointer border-2 border-gray-200 shadow-sm {{ in_array($color->id, $product->colors->pluck('id')->toArray()) ? 'selected' : '' }}" style="background-color: {{ $colorCode }}"></label>
                                    <input type="checkbox" id="color_{{ $color->id }}" name="colors[]" value="{{ $color->id }}" class="hidden" {{ in_array($color->id, $product->colors->pluck('id')->toArray()) ? 'checked' : '' }}>
                                @endforeach
                            </div>
                        </div>
                        <div class="mb-2">
                            <x-label for="maintenance" class="block text-sm font-medium text-gray-700" :value="__('Насоки за одржување')" />
                            <textarea id="maintenance" name="maintenance" rows="3" class="mt-1 rounded-md border-gray-300 w-full focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">{{ $product->maintenance }}</textarea>
                        </div>
                        <div class="mb-2">
                            ...
                            <input type="text" id="tags" name="tags" class="mt-1 p-2 border-gray-300 rounded-md w-full" placeholder="Внесете ознаки со #" value="{{ implode(', ', $product->tags->pluck('name')->toArray()) }}" />
                        </div>
                        <div class="flex m-1">
                            @foreach($product->images as $index => $image)
                                <div class="relative bg-slate-100 my-2 mr-2" id="imageBox{{$index}}" style="width: 100px; height: 80px;" data-image-id="{{ $image->id }}">
                                    <input type="file" name="images[]" id="fileInput{{$index}}" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer" accept="image/*" onchange="previewImage(event, {{$index}})" multiple>
                                    <input type="hidden" name="image_ids[]" value="{{$image->id}}">
                                    <span class="plus-icon" style="display: none">+</span>
                                    <img id="imagePreview{{$index}}" class="w-full h-full object-cover mr-5" src="{{ asset($image->image) }}" alt="Product Image">
                                </div>
                            @endforeach
                    ...
                    <div class="columns-2">
                        <div class="mb-4">
                            ...
                            <select name="product_category_id" id="product_category_id" class="border-gray-300 shadow-sm  mt-1 p-2 border rounded-md w-full" required>
...
<script>
    ...
    function previewImage(event, index) {
        ...
        reader.onload = function () {
            ...
            const oldImageIdInput = imageBox.querySelector('input[type="hidden"]');
            if (oldImageIdInput) {
                oldImageIdInput.remove();
            }
        };
    ...
    document.querySelectorAll('[id^="imagePreview"]').forEach(function(img, index) {
        img.addEventListener('click', function() {
            ...
            const imageBox = document.getElementById(`imageBox${index}`);
            const imageIdInput = document.querySelector(`input[name="image_ids[]"][value="${imageBox.dataset.imageId}"]`);
            if (imageIdInput) {
                imageIdInput.remove();
            }
            input.click();
