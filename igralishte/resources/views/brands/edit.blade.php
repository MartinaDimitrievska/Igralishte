                ...
                <form method="POST" action="{{ route('brands.update', $brand->id) }}" enctype="multipart/form-data">
                        ...
                        <div class="mb-2 ">
                            <select name="status" id="status" class="rounded-md border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                ...
                                @foreach ($statuses as $status)
                                    <option value="{{ $status->id }}" {{ $brand->status->id == $status->id ? 'selected' : '' }}>{{ $status->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="mb-2">
                            ...
                            <x-input type="text" name="name" id="name" class="mt-1 p-2 border rounded-md w-full" value="{{ $brand->name }}" autofocus />
                        </div>
                        <div class="mb-2">
                            <x-label for="description" class="block text-sm font-medium text-gray-700" :value="__('Опис')" />
                            <textarea name="description" id="description" rows="3" class="mt-1 p-2 border rounded-md w-full rounded-lg border-gray-300 shadow-sm">{{ $brand->description }}</textarea>
                        </div>
                        <div class="mb-2">
                            ...
                            <input type="text" id="tags" name="tags" class="mt-1 p-2 border-gray-300 rounded-md w-full" placeholder="Внесете ознаки со #" value="{{ implode(', ', $brand->tags->pluck('name')->toArray()) }}" />
                        </div>
                        <div class="flex m-1">
                            @foreach($brand->images as $index => $image)
                                <div class="relative bg-slate-100 mt-2 mr-2" id="imageBox{{$index}}" style="width: 100px; height: 80px;" data-image-id="{{ $image->id }}">
                                    ...
                                    <img id="imagePreview{{$index}}" class="w-full h-full object-cover mr-5" src="{{ asset($image->image) }}" alt="Brand Image">
                                </div>
                            @endforeach
